#!/usr/bin/env python3
"""
Publication figure: geometric d-series  —  energy + normalised heat capacity.

Output: plots/mergedheatplots/geometric_energy_heatcapacity.{pdf,png}
"""
import json, sys
...
# ── colormap: plasma, dark purple (low d) → bright yellow (high d) ──────────
_cmap  = plt.cm.plasma
_n     = len(D_LIST)
COLORS = {d: _cmap(0.07 + 0.80 * i / (_n - 1)) for i, d in enumerate(D_LIST)}

# ── Savitzky-Golay smoothing (applied only to C_S) ──────────────────────────
SG_WIN, SG_POLY = 9, 3
...
# ── load one d value ─────────────────────────────────────────────────────────
def load(d):
    rdir = DSERIES_DIR / f"geometric_d{d}_ordered"
...
# ── figure layout — full width, legend sits inside panel (a) ─────────────────
...
# ── legend inside panel (a), top-right where the curves are flat and sparse ──
...
# ── save ──────────────────────────────────────────────────────────────────────
out = MERGEDPLOTS_DIR / "geometric_energy_heatcapacity"
